...
**Output behavior:**
- **Quiet mode** (`CONFIG_DEBUG_NK_INVARIANTS_VERBOSE=0`): Shows only final result
  ```
  [CPU 0] Nested Kernel invariants: PASS
  ```
- **Verbose mode** (`CONFIG_DEBUG_NK_INVARIANTS_VERBOSE=1`): Shows per-invariant details
...
| `CONFIG_DEBUG_NK_INVARIANTS_VERBOSE` | 0 | Show detailed verification output |
