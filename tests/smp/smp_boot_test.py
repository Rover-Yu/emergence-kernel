#!/usr/bin/env python3
"""
SMP Boot Test - Symmetric Multi-Processing Boot Verification

Tests that the kernel boots correctly with multiple CPUs.
Verifies that the BSP (Bootstrap Processor) can successfully
start APs (Application Processors) via the trampoline mechanism.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="SMP Boot Test - Symmetric Multi-Processing Boot Verification",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s              Run with 2 CPUs (default)
  %(prog)s --cpus 4     Run with 4 CPUs
